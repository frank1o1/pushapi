...
// HMS Core > 推送服务 > API参考 下行消息
// https://developer.huawei.com/consumer/cn/doc/development/HMSCore-References-V5/https-send-api-0000001050986197-V5
type SendReq struct {
	ValidateOnly bool     `json:"validate_only,omitempty"` // 控制当前是否为测试消息，测试消息只做格式合法性校验，不会推送给用户设备，取值如下： true：测试消息 false：正式消息 默认值是false。
	Message      *Message `json:"message,omitempty"`       // 必选，推送消息结构体，message结构体中必须要有有效消息负载以及有效发送目标，具体字段请参见Message的定义。
}
...
type Message struct {
	Data         string         `json:"data,omitempty"`         // 自定义消息负载，支持普通字符串或者JSON格式字符串。样例："your data"，"{'param1':'value1','param2':'value2'}"。 消息体中有message.data，没有message.notification和message.android.notification，消息类型为透传消息。 如果用户发送的是网页应用的透传消息，那么接收消息中字段orignData为透传消息内容。
...
type AndroidNotification struct {
	...
	Icon              string                 `json:"icon,omitempty"`                // 自定义通知栏左侧小图标，此处设置的图标文件必须存放在应用的/res/raw路径下，例如“/raw/ic_launcher”，对应应用本地的“/res/raw/ic_launcher.xxx”文件，支持的文件格式目前包括png、jpg。自定义小图标规格规范请参见通知图标规范。
	Color             string                 `json:"color,omitempty"`               // 自定义通知栏按钮颜色，以#RRGGBB格式，其中RR代表红色的16进制色素，GG代表绿色的16进制色素，BB代表蓝色的16进制色素，样例：#FFEEFF。
	Sound             string                 `json:"sound,omitempty"`               // 自定义消息通知铃声，在新创建渠道时有效，此处设置的铃声文件必须存放在应用的/res/raw路径下，例如设置为“/raw/shake”，对应应用本地的“/res/raw/shake.xxx”文件，支持的文件格式包括mp3、wav、mpeg等，如果不设置使用默认系统铃声。
	DefaultSound      bool                   `json:"default_sound,omitempty"`       // 默认铃声控制开关，取值如下： true：使用系统默认铃声 false：使用sound自定义铃声
	...
	BodyLocKey        string                 `json:"body_loc_key,omitempty"`        // 显示本地化body的StringId，具体使用请参见通知消息本地化。
	BodyLocArgs       []string               `json:"body_loc_args,omitempty"`       // 本地化body的可变参数，具体使用请参见通知消息本地化。样例：["args1","args2"]
	TitleLocKey       string                 `json:"title_loc_key,omitempty"`       // 显示本地化title的StringId，具体使用请参见通知消息本地化。
	TitleLocArgs      []string               `json:"title_loc_args,omitempty"`      // 本地化title的可变参数，具体使用请参见通知消息本地化。样例：["args1","args2"]
	MultiLangKey      map[string]interface{} `json:"multi_lang_key,omitempty"`      // 消息国际化多语言参数，body_loc_key，title_loc_key优先从multi_lang_key读取内容，如果key不存在，则从APK本地字符串资源读，具体使用请参见通知消息本地化。 最多设置3种语言。
	ChannelID         string                 `json:"channel_id,omitempty"`          // 自Android O版本后可以支持通知栏自定义渠道，指定消息要展示在哪个通知渠道上，详情请参见自定义通知渠道。
	NotifySummary     string                 `json:"notify_summary,omitempty"`      // Android通知栏消息简要描述。
	Image             string                 `json:"image,omitempty"`               // 自定义通知栏消息右侧大图标URL，功能和message.notification.image字段一样，如果此处设置，则覆盖message.notification.image中的值。URL使用的协议必须是HTTPS协议，取值样例：https://example.com/image.png。
